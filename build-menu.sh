...
variant(){
  findconfig=""
  findconfig=($(ls arch/arm64/configs/xddeath69_* 2>/dev/null))
  declare -i i=1
  shift 2
...
build(){
  ...
  [ ! -d "out" ] && mkdir out
  make -j$(nproc) -C $(pwd) O=$(pwd)/out KCFLAGS=-w CONFIG_SECTION_MISMATCH_WARN_ONLY=y xddeath69_${VARIANT}_defconfig
...
anykernel3(){
  ...
  git reset --hard
  sed -i "s/ExampleKernel by osm0sis/${VARIANT} kernel by xddeath6992/g" anykernel.sh
